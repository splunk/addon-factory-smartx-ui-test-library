...
SmartConfigs = namedtuple("SmartConfigs", ['driver', 'driver_version', 'local_run', 'retry_count', 'headless_run', 'prerun_executable', 'proxy_url'])
...
def ucc_smartx_selenium_helper(request, ucc_smartx_configs, splunk, splunk_web_uri, splunk_rest_uri):
    ...
    if request.config.getoption("--proxy-url"):
        proxy_url = request.config.getoption("--proxy-ur")
