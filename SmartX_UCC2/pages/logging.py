...
class Logging(Entity):
    def __init__(self, browser, urls, session_key):
        ...
        # Components
        self.log_level =  SingleSelect(browser, {"by": By.CSS_SELECTOR, "select": ".log_level"})
        self.backend_conf = SingleBackendConf(self._get_logging_url(), session_key)

    def open(self):
        """
        Open the required page. Page(super) class opens the page by default.
        """
        self.browser.get('{}/en-US/app/Splunk_TA_microsoft-scom/configuration'.format(self.web_url))
    ...
    def _get_logging_url(self):
        ...
        """
        return '{}/servicesNS/nobody/Splunk_TA_microsoft-scom/configs/conf-microsoft_scom/logging'.format(self.mgmt_url)
